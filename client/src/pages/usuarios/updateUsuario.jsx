...
export default function UpdateUsuario() {
  ...
  if (!data || String(data.usuario.UsuarioId) !== usuarioId)
    return (
...
function UsuarioForm({ data }) {
  ...
  async function onSubmit(form) {
    ...
    try {
      ...
      const notification = notifySuccess("Usuario actualizado correctamente.");
      if (data.usuario.UsuarioId === currentUserId) {
        await notification;
  ...
  return (
    <>
      <div className="breadcrumb">
        ...
        <Link to="/usuarios">Usuarios</Link> / <span>Actualizar</span>
      </div>
      ...
      >
        <fieldset disabled={isSubmitting}>
          ...
          <p>
            La contraseña se gestiona mediante el cambio de contraseña de la
            cuenta.
          </p>
          ...
          {submitError && (
            <AlertMessage title="No se pudo actualizar el usuario">
              {submitError}
            </AlertMessage>
          )}
          <div className="table-actions">
            <button
              ...
              type="submit"
              disabled={
                isSubmitting || !disponibles.length || !data.roles.length
              }
            >
              <Save size={18} />
